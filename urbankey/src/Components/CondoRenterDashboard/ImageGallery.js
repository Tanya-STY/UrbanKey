...
const ImageGallery = ({ imageUrls }) => {
 console.log(imageUrls);
  const [isZoomed, setIsZoomed] = useState(false);
  const [bigImage, setBigImage] = useState(imageUrls[0]); // Set bigImage to the first image URL from imageUrls
  const [smallImages, setSmallImages] = useState(imageUrls.slice(1));
  const [startIndex, setStartIndex] = useState(0);
...
// const ImageGallery = ({ imageUrls }) => {
//   console.log(imageUrls);
//   // Ensure imageUrls is not empty before accessing its elements
